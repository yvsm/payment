...
if ($from === 'ali') {
    ...
    //$_GET = $data;
    $_POST = $data;
    $proxy = \zunyunkeji\payment\Client::ALIPAY;
} elseif ($from === 'wx') {
    $config = $wxConfig;
    $proxy  = \zunyunkeji\payment\Client::WECHAT;
} else {
    $config = $cmbConfig;
    $proxy  = \zunyunkeji\payment\Client::CMB;
}

$callback = new TestNotify();

try {
    $client = new \zunyunkeji\payment\Client($proxy, $config);
